...
**Status:** active — APPLIED to prod 2026-07-30 (run record: `docs/runbooks/PROD_RUNBOOK_013.md`; this line said "proposed/unapplied" until 2026-08-03, caught by the LaunchOS external sweep)
...
**Status:** active. Michael requested additional shape coverage across A/B/C and authorized release/security work concurrently. The prior one-panel-for-everything rule is superseded: preserve the nine-slice panel for flexible square/wide/tall surfaces, add one shallow control frame and one circular portrait ring per skin. Six true-alpha WebPs add 129,108 bytes; every complete skin set stays under 100 kB. No per-size image library, rasterized labels or new setting. Artwork source is independently reversible in cdc1570; browser evidence is in design/rounds/round-04/verification/.

**Security decisions:** Replace staged 014/015 with reviewed 016 rather than enable RLS while retaining a writable admin bypass and overlapping broad policies. Keep existing direct friendship APIs but enforce consent, immutable endpoints and actual mutation results through 017/client fixes. Mission assignment requires an accepted connection; purchase_reward must enforce the actual assigned recipient. Disconnecting alone does not erase existing missions/promises.

**Deletion semantics:** A confirmed account deletion removes the account's data and shared missions/proofs/created rewards; other people's credit balances remain and erased mission links are cleared. Retained offers assigned to the departing account become inactive and creator-private. Use one service-only progress marker, a real recent Auth session, write freeze, Storage API removal, then relational/Auth cleanup with confirmed completion. The UI remains feature-gated until 016/017/018 and live service checks pass. No endpoint or SQL repair is currently deployed.

**Release boundary:** The project-specific backup plus Michael's explicit production SQL review/go still applies. Current live metadata confirms serious access-control gaps; no fresh backup is available, and CLI DB initialization has a privileged-role membership error. Do not bypass that gate or call the app release-ready. Auth callback allowlist configuration was repaired under the release authorization; SMTP activation follows database repair. No test email or live account deletion was authorized/performed.
...
## 2026-09-07 — Distinct rims and private safety boundaries

**Status:** active; directly authorized and implemented in visual 95b2e83 and security fa34d0d.
**Visual:** This supersedes round 06's similar hooked silhouettes for Forged/Astral only. Keep actual shared numeral sprites and Starlight unchanged; use a hammered irregular rim for Forged and faceted octagonal glass for Astral. Filled faces avoid black voids. Two replacement WebPs add 5,334 bytes without a new sizing/rendering system.
**Privacy:** Clients select only safe profile fields, with matching staged column ACLs and restrictive counterpart visibility. The own-profile literal UPDATE still synchronizes Auth email because 016 checks it, but never reads it back. Delete unused email request logic instead of supporting a second discovery path. Clear account-scoped state on identity change.
**Blocks/reports:** A block disconnects and stops shared reads/new mutations in both directions while preserving credit balances. Unblock removes only one's own block and needs new consent. Table-level guards cover existing definer RPCs; old/new row participant groups remain separate to avoid blocking unrelated reassignment. Reports are private, bounded, caller-bound and deleted with either account; an operator must actually handle the queue. Receiving a report is not a promise of staffed moderation or an automatic ban.
**Rollout:** One default-off safety flag; no unrestricted fallback if 019 is missing. Require explicit server success, then reload after block/unblock to discard cached content. Already delivered information and existing signed URLs cannot be recalled. All new private tables join the existing deletion freeze and exact FK guard.
**Backup:** Use one schema/ACL archive preserving owners/policies/grants with verified target/hash/scope/age, exercised through real local restore. This is not an Auth/data/Storage-byte backup. Production still requires fresh backup, named Saya review and Michael's explicit go; do not equate a generic reviewer agent with Saya.
**Preview:** Resolve all Supabase imports into the fixture and use explicit dummy configuration; scan built bundles for leakage. Sample block state persists per tab with Reset, while other sample operations reset on reload. UI proof does not establish production authorization.

## 2026-09-07 — Delegated review and verified live rollout

Michael explicitly delegated technical review and authorized all release work. This supersedes earlier named Saya/human-review approval requirements above. Keep the backup/validation safeguards. Use the exact provider-documented guarded CLI-role repair after metadata capture and review, then provider-created temporary credentials in memory. Actual schema/ACL backup was restored and migrations016–019 passed rehearsal and hosted validation. Real disposable-account HTTP checks remain distinct from metadata checks and frontend deployment.
