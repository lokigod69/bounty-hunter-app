...
- **Task-lifecycle RPCs DRAFTED, awaiting Michael review (proposal 011, NO SQL applied)**: `db/proposals/011_task_lifecycle_rpcs.{md,up.sql,down.sql}` + `011_validation.sql` + `docs/runbooks/PROD_RUNBOOK_011.md`. 5 RPCs (submit_proof/reject_task/set_task_status/archive_task/delete_task), drops legacy double-credit trigger + broad assignee UPDATE policies. 4 open points flagged in the .md. After approval: apply + client refactor (useTasks.ts, missions.ts, IssuedPage.tsx) + types regen.
- **Deployed site blank — Vercel env vars missing**: the Vercel project lacks `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` for the new Supabase project (values in `.env.local`, printed in NEXT_STEP). Redeploy required after saving (Vite inlines at build). Michael says dashboard auth config (Site URL/redirects) is done now.
...
## Next actions
1. Michael: (a) ~~dashboard auth config~~ done per Michael 2026-07-10; NEW blocker: add `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` to the Vercel project env (values in `.env.local` / NEXT_STEP) and REDEPLOY; (b) then eyeball Phases 1–4 + the apple-design press/motion pass in the browser; (c) sound audition: upload/toggle alias click files, payday aliases coin.mp3 — replace with distinct audio if wanted; (d) review proposal 011 (task-lifecycle RPCs, 4 open points).
