...
ThemeContext exposes skinId/setSkinId separately from the persisted account theme. The validated bounty_skin localStorage preference sets html[data-skin] before paint and synchronizes through storage events. src/theme/skin-styles.css defines panel/control/ring/coin WebPs per finish; clipped dialogs/reward cards use tight panel crops with compensated nine-slice regions. Profile swatches preview all three. Coin composes a localized amount from ten shared ivory-platinum digit WebPs over the active skin's filled metal face; measured glyph proportions determine fit. Full accessible text, live punctuation, image-error text and forced-colors text remain. Valueless loading/ceremony tokens use the same face without a denomination. The original four coin assets are removed. AppButton keys a decorative span per press to restart one short CSS rim glint without timers. No new backend fields or runtime dependencies for artwork.
...
## 2026-09-07 staged contact safety and backup workflow

Proposal 019 adds bounty_private.user_blocks, user_reports and safety_budgets. Public caller-bound RPCs provide lookup/block/list/report; private helpers enforce bilateral block checks under the account deletion advisory locks. Restrictive policies and write guards cover existing content/RPC/Storage paths, and the new tables participate in 018 freeze/cascade and exact schema assertions. The reward notifier requires a service-only pair preflight.

PersonSummary separates public identity from own-profile preferences. Explicit projections avoid email/role SELECT; bootstrap/save remain compatible with 016. AuthContext exposes only the active account's profile; Friends remounts by account. contactSafety.ts owns the staged type overlay and default-off VITE_CONTACT_SAFETY_ENABLED gate. PersonSafety is shared by People, mission headers and reward cards. Blocking reloads the document to discard stale content. Twelve locale dictionaries carry the safety copy.

The preview Vite resolver normalizes relative import paths before routing Supabase to its fictional adapter, and defines only dummy backend configuration. Sample safety state is per-tab session storage, reset through the fixture toolbar. No production config imports this adapter.

scripts/prod/schema_backup_guard.ps1 validates custom pg_dump inventory and the target/hash/scope/age manifest used by backup_schema.ps1 and apply_sql.ps1. The test runner restores an actual archive into a second local database. protocol/backup-release.bat supplies the Windows local-password handoff. Backups/manifests are ignored; no password or production backup is committed.
