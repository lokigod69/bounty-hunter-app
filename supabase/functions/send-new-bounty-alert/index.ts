...
// V1 disabled: assignment email notifications are not part of the launch path.
...
serve(async (req: Request) => {
  ...
  return new Response(
    JSON.stringify({
      error: 'send-new-bounty-alert is disabled for V1. Resend-backed notification work should be reintroduced behind explicit authorization checks.',
    }),
    {
      status: 410,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
