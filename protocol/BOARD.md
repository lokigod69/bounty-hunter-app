...
## ⚠️ WAITING ON YOU
- **Design V2 Wave B browser/device smoke + commit** — accept → in progress → proof; text/image/video/PDF evidence both sides; Sent back remains on Issued; archive has both roles/no alarm; realtime refresh preserves open dossier; nested Escape/focus works. Code is verified but intentionally uncommitted.
...
|---|---|---|---|---|
| main | Design V2 — Wave A committed; Wave B verified/uncommitted | Jul 24: Wave B lifecycle truth, accept verb, EvidencePanel, SWR lists, modal a11y/stacking, complete archive, small truth fixes; tsc 0 / 89 tests / lint 0 errors / build pass | YOU: Wave B browser/device smoke, then commit. Proposal 012 remains SQL-gated; older dashboard/Mac tasks parked below | 🟢 active |
...
- 2026-07-24 **Design V2 Wave B implemented + verified (uncommitted)**: sent-back truth/section; Accept contract; text + inline private EvidencePanel; stale-while-revalidate lists; modal focus/stacking/material; full archive; StatsRow/payday truth fixes; tsc 0 / 89 tests / lint 0 errors / build pass
