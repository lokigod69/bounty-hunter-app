      ...
      <div className="bg-gray-900 w-full h-[95vh] md:h-auto md:max-w-lg rounded-t-2xl md:rounded-xl md:border md:border-gray-700 flex flex-col">
        <form onSubmit={handleSubmit} className="flex flex-col h-full">
          ...
          <div className="flex justify-between items-center p-4 border-b border-gray-700/50 flex-shrink-0">
            <h2 className="text-xl font-bold text-white">Edit Bounty</h2>
          ...
          <div className="flex-grow overflow-y-auto p-4 md:p-6 space-y-6">
            <input
              ...
              className="w-full p-3 bg-gray-800/80 border border-gray-700 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition"
              required
              maxLength={100}
            />
            <textarea
              ...
              className="w-full p-3 bg-gray-800/80 border border-gray-700 rounded-lg h-24 resize-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition"
            />
            <input
              type="number"
            ...
            <div>
              <div className="flex items-center justify-center space-x-4 mb-3">
                <button type="button" onClick={() => setImageType('emoji')} className={`px-4 py-2 rounded-lg text-sm font-semibold transition ${imageType === 'emoji' ? 'bg-teal-500 text-black' : 'bg-gray-700 text-white'}`}>Use Emoji</button>
                <button type="button" onClick={() => setImageType('url')} className={`px-4 py-2 rounded-lg text-sm font-semibold transition ${imageType === 'url' ? 'bg-teal-500 text-black' : 'bg-gray-700 text-white'}`}>Use Image URL</button>
              </div>
              {imageType === 'emoji' ? (
                ...
                <div>
                  <input
                    ...
                    className={`w-full p-3 bg-gray-800/80 border border-gray-700 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition ${imageUrlError ? 'border-red-500' : ''}`}
                  />
          ...
          <div className="p-4 bg-gray-900/80 backdrop-blur-sm border-t border-gray-700/50 flex justify-end space-x-4 flex-shrink-0">
            <button type="button" onClick={onClose} className="px-6 py-3 rounded-lg text-white bg-gray-700 hover:bg-gray-600 transition font-semibold">Cancel</button>
