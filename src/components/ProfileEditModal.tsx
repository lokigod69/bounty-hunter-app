...
export default function ProfileEditModal({ isOpen, onClose }: ProfileEditModalProps) {
  ...
  return (
    ...
    >
      <div 
        className="glass-card w-full max-w-md bg-gray-900/80 rounded-2xl border border-gray-700 flex flex-col animate-fade-in-up max-h-[90vh]"
        onClick={(e) => e.stopPropagation()} // Prevent click inside from closing
      >
        {/* Modal Header */}
        <div className="flex items-center justify-between p-4 border-b border-gray-700/50 flex-shrink-0">
          <div className="w-8"></div> {/* Spacer */}
          <h2 className="text-xl font-bold text-center flex-grow">{t('profile.edit')}</h2>
          <button onClick={onClose} className="w-8 p-1.5 bg-gray-800/60 hover:bg-gray-700/80 rounded-full transition-colors" aria-label="Close profile edit modal">
            <X size={20} />
          </button>
        </div>
