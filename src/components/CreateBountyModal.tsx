...
const CreateBountyModal: React.FC<CreateBountyModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const validateImageUrl = (url: string) => {
    ...
    if (!isValid) {
      setImageUrlError('Please enter a valid image URL (e.g., jpg, png, unsplash).');
      return false;
    }
      ...
      <div className="bg-gray-900 w-full h-[95vh] md:h-auto md:max-w-lg rounded-t-2xl md:rounded-xl md:border md:border-gray-700 flex flex-col">
        <form onSubmit={handleSubmit} className="flex flex-col h-full">
          ...
          <div className="flex justify-between items-center p-4 border-b border-gray-700/50 flex-shrink-0">
            <h2 className="text-xl font-bold text-white">Create a New Bounty</h2>
          ...
          <div className="flex-grow overflow-y-auto p-4 md:p-6 space-y-6">
            ...
            <input
              type="text"
              placeholder="Bounty Name (e.g., Design a Logo)"
              value={name}
            ...
            <textarea
              ...
              className="w-full p-3 bg-gray-800/80 border border-gray-700 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition h-24"
              required
            />
