...
const LoginBackdrop = () => (
  <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden" aria-hidden="true">
    <img
      ...
      alt=""
      draggable={false}
      className="absolute inset-[-4px] h-[calc(100%+8px)] w-[calc(100%+8px)] object-cover"
      style={{
        opacity: 0.22,
        filter: 'saturate(0.7) blur(2px)',
        maskImage: 'radial-gradient(ellipse at center, black 0%, rgba(0,0,0,0.88) 42%, transparent 82%)',
        WebkitMaskImage: 'radial-gradient(ellipse at center, black 0%, rgba(0,0,0,0.88) 42%, transparent 82%)',
      }}
    />
  </div>
);

const LoginFrame: React.FC<React.PropsWithChildren> = ({ children }) => (
        ...
        <div className="mt-6">
          ...
          {showOtherOptions && (
            <div className="mt-4 p-4 rounded-lg border border-white/10">
              {/* Native v1 uses email until an Apple-compliant social sign-in is shipped. */}
              {!Capacitor.isNativePlatform() && (<button
                  onClick={handleGoogleLogin}
                  disabled={loading}
                  className="w-full min-h-[48px] py-3 px-4 mb-4 rounded-xl bg-[#131314] hover:bg-[#1f1f20] border border-white/15 text-white font-semibold transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
                >
